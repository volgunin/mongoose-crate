...
class Crate {
  ...
  plugin (schema, options) {
    ...
    schema.methods.attach = function (field, attachment, callback) {
      ...
      if (Buffer.isBuffer(attachment.path)) {
        // make sure the name exists
        if (!attachment.name) {
          tasks.push(next => {
            next(new Error('Attachment has no name property!'))
          })
        }
        ...
        if(!attachment.type) {
          tasks.push(next => {
            self._magic.detect(attachment.path, (error, type) => {
        ...
        if(!attachment.size) {
          attachment.size = attachment.path.length
        }
      } else {
