...
Crate.prototype.plugin = function(schema, options) {
  ...
  schema.methods.attach = function(field, attachment, callback) {
    ...
    if (attachment.path) {
      ...
      if(!attachment.name) {
        return callback(new Error('Attachment has no name property!'))
      }

      // make sure there's a filesize
      if(!attachment.size) {
      ...
      if(!attachment.type) {
        tasks.push(function(callback) {
          self._magic.detect(attachment.data, function(error, type) {
            attachment.type = type
            callback(error)
          })
        }.bind(this))
      ...
      return callback(new Error('Attachment has no path or data property!'))
    }
